...
(function() {
  var BUMP_FACTOR, DEFAULT_HEARTBEAT, REGISTRY_PORT, Registry, SERVICE_HOST, VERBOSE, argv, checkServices, deregisterService, deregisteredRemoteService, findServices, foundRemoteServices, getHealthyServiceByName, getRemoteServiceByName, getService, getServiceByClientId, getServiceById, heartbeats, host, isHealthy, join, join_string, log, minimist, port, ref, registerService, registered, registeredRemoteService, registry, registry_methods, registry_options, remote_registered, somata, summarizeServices,
  ...
  registerService = function(client_id, service_instance, cb) {
    var heartbeat_interval, service_id, service_name;
  ...
  summarizeServices = function(services) {
    var results, service_id, service_instance, service_instances, service_name;
    if (services == null) {
      services = registered;
    }
    ...
    for (service_name in services) {
      ...
      results.push((function() {
        ...
        for (service_id in service_instances) {
          service_instance = service_instances[service_id];
          results1.push(console.log('    * ' + service_id));
        }
        return results1;
      })());
    }
  ...
  foundRemoteServices = function(err, remote_services) {
    var service_id, service_instance, service_instances, service_name;
    for (service_name in remote_services) {
      service_instances = remote_services[service_name];
      remote_registered[service_name] || (remote_registered[service_name] = {});
      for (service_id in service_instances) {
        service_instance = service_instances[service_id];
        remote_registered[service_name][service_id] = service_instance;
      }
    }
    return summarizeServices(remote_registered);
  };

  registeredRemoteService = function(service) {
    var name;
  ...
  deregisteredRemoteService = function(service) {
    delete remote_registered[service.name][service.id];
    registry.publish('deregister', service);
    return summarizeServices(remote_registered);
  };

  join = function(host, port, cb) {
    var join_client;
    join_client = new somata.Client({
      registry_host: host,
      registry_port: port
    });
