...
(function() {
  var BUMP_FACTOR, DEFAULT_HEARTBEAT, REGISTRY_HOST, REGISTRY_PORT, REGISTRY_PROTO, Registry, VERBOSE, argv, checkServices, deregisterService, deregisteredRemoteService, findServices, foundRemoteServices, getHealthyServiceByName, getRemoteServiceByName, getService, getServiceByClientId, getServiceById, heartbeats, host, isHealthy, join, join_string, log, minimist, port, ref, registerService, registered, registeredRemoteService, registry, registry_methods, registry_options, remote_registered, somata,
  ...
  join = function(remote_registry, cb) {
    ...
    join_client = new somata.Client({
      ...
      registry_port: remote_registry.port || 8420
    });
    join_client.registry_connection.on('connect', function() {
      return join_client.remote('registry', 'findServices', foundRemoteServices(remote_registry));
    });
    ...
    return cb(null, "Joining to " + remote_registry.host + ":" + remote_registry.port + "...");
  };
